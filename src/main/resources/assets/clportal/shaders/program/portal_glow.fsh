...
// from portal.fsh, to match the color
vec4 color_at(vec2 tex_coord, float fractal_value) {
    ...
    vec2 normalized_pos = tex_coord / portal_size.xz * 2.;
...
void main() {
    vec3 normalized_pos = tex_coord / portal_size * 2.;
    float height1 = perlin(vec3(tex_coord.xz, time / 50.));
    float height2 = perlin(vec3(tex_coord.xz * 3., time / 20.)) * .7;
    float fractal_height = fractal_noise(vec3(tex_coord.xz * 5., time / 20.)) * (.3 + fizzle_anim);
    height1 += fractal_height;
    height2 += fractal_height;
    float at_current_height1 = smoothstep(min(1., height1), height1 - .3, normalized_pos.y / 2.);
    float at_current_height2 = smoothstep(min(1., height2), height2 - .3, normalized_pos.y / 2.);

    float fractal_value = fractal_noise(vec3(tex_coord.xz * 10., time / 50.));
    vec2 coord = floor(tex_coord.xz * 16. + vec2(.5)) / 16.;
    vec4 color_at_coord = color_at(coord * (2. - open_anim), fractal_value);
    color_at_coord.a *= at_current_height1 + at_current_height2 * open_anim;

    gl_FragColor = mix(vec4(0), color_at_coord, smoothstep(0., .1 + fizzle_anim * .9, normalized_pos.y) * (1. - fizzle_anim));
}
